...
type ProjectsApi(baseUrl: string, auth: Authentication, http: IHttpClient) =
    ...
    member _.List(?search: string, ?simple: bool) : Async<Project array> =
        async {
            let query =
                [
                    "search", search
    ...
    member _.Get(projectId: int) : Async<Project> =
        async {
    ...
    member this.Search(search: string) : Async<Project array> =
