...
- **Prefer an explicit `let mutable` field + a hand-written `member` property over `member val … with get, set`.** Fable renders an F# auto-property's compiler-generated backing field as `this["Name@"]` — the `@` is F#'s backing-field naming convention and is not a valid JS identifier, so it surfaces as an ugly string-keyed access in the transpiled output. An explicit `let mutable name = …` paired with `member _.Name with get () = name and set v = name <- v` transpiles to a clean, normally-named field. Use plain get-only `member _.X = …` for computed/immutable properties.
